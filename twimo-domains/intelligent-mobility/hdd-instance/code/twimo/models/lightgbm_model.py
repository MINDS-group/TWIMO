...
def _make_lightgbm_classifier(n_classes: int):
    """Return LightGBM classifier with tuned hyperparameters."""
    try:
...
def train_lightgbm_multihead(
    ...
    """Train one LightGBM classifier per horizon.
    ...
    for steps in u_steps:
        ...
        # Remap labels to contiguous [0..n_local-1] to satisfy LightGBM's requirement.
        le = LabelEncoder()
        ...
        clf = _make_lightgbm_classifier(n_local)
        ...
        print(f"  [LightGBM] horizon={steps} steps ({secs_str}), val_acc={acc:.4f}")

    ckpt = {
        "type": "lightgbm_window_stats_v1",
...
def predict_lightgbm_multihead(ckpt: Dict, X: np.ndarray, cfg: ManeuverConfig) -> Dict[int, np.ndarray]:
...
def save_lightgbm_checkpoint(ckpt: Dict, path) -> None:
    import joblib
    joblib.dump(ckpt, path)


def load_lightgbm_checkpoint(path) -> Dict:
    import joblib
