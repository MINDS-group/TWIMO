...
def _make_classifier(n_classes: int):
    """Return a scikit-learn model with predict_proba.

    Preference order:
    - xgboost.XGBClassifier (if installed)
    - sklearn.ensemble.RandomForestClassifier

    We avoid heavy hyperparam tuning: this is meant as a *baseline*.
    """
    try:
        import xgboost as xgb  # type: ignore

        # multi:softprob gives calibrated-ish probabilities
...
def train_baseline_multihead(
    ...
    """Train one baseline classifier per horizon.
...
def predict_baseline_multihead(ckpt: Dict, X: np.ndarray, cfg: ManeuverConfig) -> Dict[int, np.ndarray]:
...
def save_baseline_checkpoint(ckpt: Dict, path) -> None:
    import joblib

    joblib.dump(ckpt, path)


def load_baseline_checkpoint(path) -> Dict:
    import joblib
